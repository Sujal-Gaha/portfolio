...
Working with try catch every time can be tedious and repetitive. We can use `npm` packages like `express-async-handler` but why bother installing third party packages when we can make our own custom express async handler.
