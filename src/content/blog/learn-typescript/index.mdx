...
interface Car { // Merges with the previous declaration
  year: number;
}
...
class Employee implements PersonInterface { // Works with interface
  name: string = "Charlie";
  age: number = 50;
}
...
class Manager implements PersonType { // Actually works in recent TS versions, but interfaces are cleaner 
  name: string = "Dave";
  age: number = 60;
}
